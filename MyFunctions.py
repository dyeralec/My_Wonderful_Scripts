...
def CreateShapefile_Memory(name, srs, geom, schema=None):

    outDriver = ogr.GetDriverByName('MEMORY')
    # create output shape file
    outDataSource = outDriver.CreateDataSource('memData_' + format(name))
    outLyr = outDataSource.CreateLayer(name, srs, geom)

    if schema:
        # Add input Layer Fields to the output Layer
        for feat in schema:
            fld = ogr.FieldDefn(feat[0], feat[1])
            outLyr.CreateField(fld)

    del outLyr
    return outDataSource


def RasterToPoints(inRaster, srs, schema=None):
    ...
    for i in range(0, data.shape[0]):
        for j in range(0, data.shape[1]):
            ...
            #         print(y_dist_j)
            coords.append([x_dist_i, y_dist_j])
    ...
    for c in coords:
        ...
        point.AddPoint(c[0], c[1])
        outFeat.SetGeometry(point)
        outFeat.SetField('Longitude', c[0])
        outFeat.SetField('Latitude', c[1])
        memLyr.CreateFeature(outFeat)
        del outFeat

    del memLyr
    return memLyr_ds, coords

def SpatialFilter_Remove(inShp, filterShp, keep_within=True):

    if isinstance(inShp, str):
        driver = ogr.GetDriverByName('ESRI Shapefile')
        inShpDs = driver.Open(inShp, 0)
    else:
        inShpDs = inShp

    inLyr = inShpDs.GetLayer()

    if isinstance(filterShp, str):
        driver = ogr.GetDriverByName('ESRI Shapefile')
        filterDs = driver.Open(filterShp, 0)
    else:
        filterDs = filterShp

    filterLyr = filterDs.GetLayer()

    # get geomerty from filter
    featureCount = filterLyr.GetFeatureCount()
    if featureCount > 1:
        print('Feature count for filter shapfile is greater than 1. Spatial filter will only\n',
              'perform on the first feature.')
    filterFeat = filterLyr.GetNextFeature()
    geom = filterFeat.geometry().Clone()

    inLyr.SetSpatialFilter(geom)

    # ldefn = inLyr.GetLayerDefn()
    # for n in range(ldefn.GetFieldCount()):
    #     fdefn = ldefn.GetFieldDefn(n)
    #     print(fdefn.name)

    # gather feature FIDs that are left after the spatial filter
    FIDs = []
    ...
    # if keep_within=True, then remove all features outside of the spatial filter
    # else, remove all features inside of the spatial filter
    for feature in inLyr:
        fid = feature.GetFID()
        if keep_within == True:
            if fid not in FIDs:
                inLyr.DeleteFeature(fid)
        elif keep_within == False:
            if fid in FIDs:
                inLyr.DeleteFeature(fid)
        else:
            print('Input for attribute keep_within not valid')
...
def RasMemToTiff(ds, output_ras):
    '''Converts raster datasource in memory to a geotiff'''

    # Get original no data value
    band1 = ds.GetRasterBand(1)
    NoDataVal = band1.GetNoDataValue()

    # Collect all raster information and delete ds from memory
    raster_data = ds.ReadAsArray()
    geo_transform = ds.GetGeoTransform()
    x_size = ds.RasterXSize  # Raster xsize
    y_size = ds.RasterYSize  # Raster ysize
    srs = ds.GetProjectionRef()  # Projection
    ds = None

    # Need a driver object. By default, we use GeoTIFF
    driver = gdal.GetDriverByName("gtiff")
    dataset_out = driver.Create(output_ras, x_size, y_size, 1, gdal.GDT_Float32)
    driver = None
    dataset_out.SetGeoTransform(geo_transform)
    dataset_out.SetProjection(srs)
    band_out = dataset_out.GetRasterBand(1)
    band_out.SetNoDataValue(NoDataVal)
...
def ExtractValuesByLocations(rasDS, lats, lons):
    ''' Pull raster values from rasDS based on location (latitude, longitude), returns list of values'''

    values = []

    # Pull band & values of interest from raster
    band = rasDS.GetRasterBand(1)
    cols = rasDS.RasterXSize
    rows = rasDS.RasterYSize

    transform = rasDS.GetGeoTransform()
    ...
    # Loop through locations and pull raster values at locations
    l = 0
    for lat in lats:
        lon = lons[l]
        ...
        col = int((float(lat) - xOrigin) / pixelWidth)
        row = int((yOrigin - float(lon)) / pixelHeight)

        result = float(data[row][col])
...
def ExtractValueByLocation(rasDS, lat, lon):
    ''' Pull raster values from rasDS based on location (latitude, longitude), returns list of values'''
    ...
    col = int((float(lat) - xOrigin) / pixelWidth)
    row = int((yOrigin - float(lon)) / pixelHeight) * -1
...
def RasToMem(inRas):
    '''Copies raster into memory, returns raster datasource'''
    r_datasource = gdal.Open(inRas)

    mem_driver = gdal.GetDriverByName("MEM")
    ds = mem_driver.CreateCopy("", r_datasource)

    return ds


def ReprojectRaster_WGS84(inRaster):
    '''Reprojects raster (string path and name) to WGS84'''
    r_datasource = gdal.Open(inRaster)

    ds = gdal.Warp('', r_datasource, dstSRS='EPSG:4326', format='VRT',
                   outputType=gdal.GDT_CFloat32)  # , xRes=0.00892857142857143, yRes=0.0089285714285714

    return ds


def CheckSRS_WGS84(inData):
    '''Checks if input data is in WGS 84 geographic projection, returns True/False'''

    WGS84_srs = [
        'GEOGCS["WGS 84",\n    DATUM["WGS_1984",\n        SPHEROID["WGS 84",6378137,298.257223563,\n            AUTHORITY["EPSG","7030"]],\n        AUTHORITY["EPSG","6326"]],\n    PRIMEM["Greenwich",0,\n        AUTHORITY["EPSG","8901"]],\n    UNIT["degree",0.0174532925199433,\n        AUTHORITY["EPSG","9122"]],\n    AUTHORITY["EPSG","4326"]]',
        'GEOGCS["WGS 84",\n    DATUM["WGS_1984",\n        SPHEROID["WGS 84",6378137,298.257223563,\n            AUTHORITY["EPSG","7030"]],\n        AUTHORITY["EPSG","6326"]],\n    PRIMEM["Greenwich",0],\n    UNIT["degree",0.0174532925199433],\n    AUTHORITY["EPSG","4326"]]']

    if inData.split('.')[-1] == 'shp':
        driver = ogr.GetDriverByName("ESRI Shapefile")
        datasource = driver.Open(inData, 0)
        lyr = datasource.GetLayer()
        srs = str(lyr.GetSpatialRef())

    else:  # Assumed raster
        r_datasource = gdal.Open(inData)
        r_srs = r_datasource.GetProjection()
        srs = osr.SpatialReference(r_srs)
...
def CheckSRS(inData_List):
    ''' Check if all items in inData_List share the same srs or if there are differences '''
    SRS_list = []
    for d in inData_List:
        ...
        if d.split('.')[-1] == 'shp':
            driver = ogr.GetDriverByName("ESRI Shapefile")
            datasource = driver.Open(d, 0)
            lyr = datasource.GetLayer()
            SRS_list.append(str(lyr.GetSpatialRef()))

        else:  # Assumed raster
            r_datasource = gdal.Open(d)
            r_srs = r_datasource.GetProjection()
            SRS_list.append(osr.SpatialReference(r_srs))

    return all(x == SRS_list[0] for x in SRS_list)

def checkGeometry(inputShp):

    # get layer from data source
    d_in = ogr.GetDriverByName('ESRI Shapefile')
    ds_in = d_in.Open(inputShp, 0)
    l_in = ds_in.GetLayer()
...
def checkPointWithinMask(inputShp, maskFile):
    ...
    points = [pt for pt in records(inputShp)]
    multipol = records(maskFile)
...
def getRasterValuesAtPoints(inputShp, inputRasters):

    outputValues = []

    # open shapefile
    shp_dr = ogr.GetDriverByName('ESRI Shapefile')
    shp_ds = shp_dr.Open(inputShp)
    ...
    for p in shp_lyr:
        ...
        for raster in inputRasters:
            ...
            px = int((mx - geotrans[0]) / geotrans[1])  # x pixel
            py = int((my - geotrans[3]) / geotrans[5])  # y pixel

            intval = band.ReadAsArray(px, py, 1, 1)
            ptValues.append(intval[0])

        outputValues.append(ptValues)
