...
output_spreadsheet = r"P:\05_AnalysisProjects_Working\Offshore Infrastructure and Incidents REORG\03_Analysis\Incidents\Incident Counts by Type\Struct_Weather_Incs_2008-2018_Stats_Grouped_Yearly.csv"
type_field_index = 16
date_field_index = 0
date_format = '%m/%d/%Y'
beginning_year = 2008
...
fixed_group= ["FIXED", "WP", "CAIS", "CT"]
MOPU_group = ["SEMI", "SPAR", "TLP", "MTLP", "FPSO", "SSMNF", "MOPU"]
...
header = ['Year',record_type]

# get list of all the possible rig types
RigTypes = ["FIXED", "MOPU", "UNKNOWN"]
# for row in records:
# 	if row != records[0]:
	...
	for record in records:
		if record != records[0]:
			...
			if (y == year):
				...
				if rig_type in fixed_group:
					type_list.append("FIXED")
				if rig_type in MOPU_group:
					type_list.append("MOPU")
				if rig_type == "UNKNOWN":
