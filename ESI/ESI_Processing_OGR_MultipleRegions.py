...
The user will need to set these variables before running the code:
    ...
    - dissolveField_biofileIndex: the number to index the dissolve field from the base table. 'OBJECTID' field does NOT
            count towards the index number, so exclude this column from the index count
...
Important implications of this script:
    ...
    and dealt with like the other shapefiles at the end of the script.
...
def GrabUniqueValuesFromField(inGDB,inFileName,field):
    """
...
if __name__ == '__main__':
    ...
    MainFolder = r'P:\02_DataWorking\Atlantic\Impacts\ESI_ByState\FloridaESI'
    
    for subdir, dirs, files in os.walk(MainFolder):
        for dir in dirs:
            ParentFolder = os.path.join(subdir, dir)
            print('parent: ' + ParentFolder)
            for subdir1, dirs1, files1 in os.walk(ParentFolder):
                for dir1 in dirs1:
                    if dir1.endswith('.gdb'):
                        ESI_GDB = os.path.join(subdir1, dir1)
                        print('GDB: ' + ESI_GDB)
                            ...
                            for value in uniqueValues_RARNUM:
                                ...
                                try:
                                    ...
                                    if os.path.exists(new_features_folder + '\\' + feature + '_' + format(additional_file_index) + '.shp'):
                                        ...
                                        finalLayer = finalDataSource.CreateLayer(feature + '_' + format(additional_file_index), FeatureLayer.GetSpatialRef(), setGeomTo)
